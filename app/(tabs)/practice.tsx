...
function getDifficultyOptions(colors: ThemeColors) {
  return [
    { id: 'all' as DifficultyFilter,    label: 'הכל',    icon: '🎯', color: colors.primary },
    { id: 'easy' as DifficultyFilter,   label: 'קל',     icon: '🟢', color: colors.success },
    { id: 'medium' as DifficultyFilter, label: 'בינוני', icon: '🟡', color: colors.warning },
    { id: 'hard' as DifficultyFilter,   label: 'קשה',    icon: '🔴', color: colors.danger },
  ];
}

function getFreeModes(colors: ThemeColors) {
  return [
    {
      id: 'practice',
      icon: '📖',
...
}: { mode: ReturnType<typeof getFreeModes>[0]; isSelected: boolean; onPress: () => void }) {
  const colors = useColors();
  const styles = React.useMemo(() => makeStyles(colors), [colors]);
...
function SimStat({ icon, label, value }: { icon: string; label: string; value: string }) {
  ...
  return (
    <View style={ssStyles.wrap}>
      ...
      <Text style={[ssStyles.value, { color: colors.text }]}>{value}</Text>
...
const ssStyles = StyleSheet.create({
  ...
  value: { fontFamily: FontFamily.bold, fontSize: FontSize.base },
