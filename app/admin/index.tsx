...
const ADMIN_SECTIONS = [
  {
    ...
    icon: '📋',
    label: 'מאגר שאלות',
    desc: 'CRUD, סינון, חיפוש, bulk actions',
  ...
  },
  {
    ...
    icon: '✅',
    label: 'תור ולידציה',
    desc: 'אישור/דחיית שאלות ממתינות',
  ...
  },
  {
    ...
    icon: '🏗️',
    label: 'בניית סימולציה',
    desc: 'תבניות מבחן חכמות עם כללים',
    color: Colors.warning,
    gradient: Colors.gradients.gold,
    route: '/admin/simulation-builder',
  },
  {
    id: 'analytics',
    icon: '📊',
    label: 'אנליטיקס',
    desc: 'גרפים, התפלגויות, מגמות',
    color: '#0EA5E9',
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <LinearGradient colors={['#0F172A', '#1E293B']} style={styles.header}>
          ...
          <Pressable onPress={() => { logout(); router.replace('/(tabs)'); }} style={styles.logoutBtn}>
          ...
          >
            <LinearGradient colors={Colors.gradients.gold} style={styles.alertBannerGrad}>
              ...
              <Text style={styles.alertText}>
                {pendingCount} שאלות ממתינות לאישור — לחץ לאישור
              </Text>
        ...
        <Pressable
          ...
          style={({ pressed }) => [styles.quickAdd, pressed && { opacity: 0.9 }]}
        >
...
function MiniStat({ label, value, icon, color, alert }: {
  label: string; value: number | string; icon: string; color: string; alert?: boolean;
}) {
  return (
...
function PinScreen({ pin, setPin, error, onSubmit }: {
  pin: string; setPin: (v: string) => void; error: boolean; onSubmit: () => void;
}) {
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#0F172A' }}>
      ...
      >
        ...
        <Text style={{ fontFamily: FontFamily.heading, fontSize: FontSize['2xl'], color: '#fff', marginBottom: 8 }}>
          כניסה למנהל
        </Text>
        ...
        />
        {error && (
          <Text style={{ color: Colors.danger, fontFamily: FontFamily.medium, marginTop: 12 }}>
            קוד שגוי — נסה שוב
          </Text>
        )}
        <Pressable
        ...
        >
          <Text style={{ fontFamily: FontFamily.bold, fontSize: FontSize.lg, color: '#fff' }}>
            כניסה
          </Text>
        </Pressable>
        <Pressable onPress={() => router.back()} style={{ marginTop: 20 }}>
          <Text style={{ fontFamily: FontFamily.regular, fontSize: FontSize.sm, color: '#64748B' }}>
            ← חזרה לאפליקציה
          </Text>
        </Pressable>
      </KeyboardAvoidingView>
